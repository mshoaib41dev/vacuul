...
interface PaginationProps extends Partial<Omit<PaginationRootProps, "children">> {
    ...
    hasPrevious?: boolean;
    /** Callback function for next page. */
    onNext?: () => void;
...
export const PaginationPageDefault = ({ 
    ...
    // Use custom callbacks if provided, otherwise fall back to built-in logic
    const handlePrevious = onPrevious || (() => props.onPageChange?.(Math.max(1, page - 1)));
    const handleNext = onNext || (() => props.onPageChange?.(Math.min(total, page + 1)));
    
    // Use custom hasNext/hasPrevious if provided, otherwise calculate from page/total
    const canGoPrevious = hasPrevious !== undefined ? hasPrevious : page > 1;
    const canGoNext = hasNext !== undefined ? hasNext : page < total;
