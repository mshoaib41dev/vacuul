...
import { Link as AriaLink } from "@/components/base/link/link";
...
export const NavItemBase = ({ current, type, badge, href, icon: Icon, children, truncate = true, onClick, disabled = false }: NavItemBaseProps) => {
    const iconElement = Icon && (
        <Icon
            aria-hidden="true"
            className={cx("mr-2 size-5 shrink-0 text-fg-quaternary transition-inherit-all", disabled && "text-fg-disabled")}
        />
    );
    ...
    const labelElement = (
        <span
            className={cx(
                ...
                disabled && "text-disabled group-hover:text-disabled",
            )}
        >
            {children}
        </span>
    ...
    if (disabled) {
        return (
            <div
                aria-disabled="true"
                className={cx(
                    type === "collapsible-child" ? "py-2 pr-3 pl-10" : "px-3 py-2",
                    styles.root,
                    styles.rootDisabled,
                    current && styles.rootSelected,
                )}
            >
                {type !== "collapsible-child" && iconElement}
                {labelElement}
                {externalIcon}
                {badgeElement}
            </div>
